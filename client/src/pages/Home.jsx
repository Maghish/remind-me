...
function AllTasksWrap() {
  ...
  useEffect(() => {
    async function getAllTasks() {
      try {
        axios
          .post("/auth", { token: GetCookie("userToken") })
          .then((currentUser) => {
            const currentUserId = currentUser.data.userData._id;
            axios
              .post("/api/gettasks", { creatorID: currentUserId })
              .then((res) => {
  ...
  if (allTasks.length === 0) {
    return (
      <div className="w-full min-h-full flex items-center justify-center">
        <div className="border-2 rounded-lg border-stone-600 p-4">
          <span className="font-mono text-white">No tasks found</span>
        </div>
      </div>
    );
  } else {
  ...
  return (
    <>
      ...
      <div className="w-full min-h-screen flex flex-col items-center">
        <p className="font-mono text-white text-4xl pt-9">
